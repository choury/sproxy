...
void SocketRWer::connect() {
    ...
    if(protocol == Protocol::TCP) {
        int fd = Connect(&addr, SOCK_STREAM, assign_src);
        if (fd < 0) {
        ...
        int fd = Connect(&addr, SOCK_DGRAM, nullptr);
        if (fd < 0) {
            con_failed_job = UpdateJob(std::move(con_failed_job),
                                       ([this, error = errno]{connectFailed(error);}), 0);
            return;
        ...
        int fd = Connect(&addr, SOCK_DGRAM, assign_src);
        if (fd < 0) {
            con_failed_job = UpdateJob(std::move(con_failed_job),
                                       ([this, error = errno]{connectFailed(error);}), 0);
            return;
