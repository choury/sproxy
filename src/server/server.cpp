...
int main(int argc, char **argv) {
    ...
    for(struct bind_list* node = opt.listen_list; node; node = node->next) {
        ...
        if(strcmp(info.protocol, "http") == 0 || strcmp(info.protocol, "ssl") == 0) {
            // TCP listen
            if(info.systemd_fd >= 0) {
                fd[0] = info.systemd_fd;
            } else if(strcmp(info.hostname, "localhost") == 0) {
                if(ListenLocalhostTcp(info.port, fd, nullptr) < 0) {
                    return -1;
                }
            } else {
                fd[0] = ListenTcpD(&info, nullptr);
                if (fd[0] < 0) {
                    return -1;
                }
            }
            ...
            if(info.systemd_fd >= 0) {
                ...
                if(ListenLocalhostUdp(info.port, fd, nullptr) < 0) {
                    return -1;
                }
            } else {
                fd[0] = ListenUdpD(&info, nullptr);
                if(fd[0] < 0) {
                    return -1;
                }
            }
            ...
            if(info.sni_mode) {
                servers.emplace_back(std::make_shared<Quic_sniServer>(fd[0], info.port));
                if(fd[1] >= 0) {
                    servers.emplace_back(std::make_shared<Quic_sniServer>(fd[1], info.port));
                }
                if(info.systemd_fd >= 0) {
                ...
                servers.emplace_back(std::make_shared<Quic_server>(fd[0], info.port, ctx));
                if(fd[1] >= 0) {
                    servers.emplace_back(std::make_shared<Quic_server>(fd[1], info.port, ctx));
                }
                if(info.systemd_fd >= 0) {
                    LOG("listen on systemd fd %d for quic\n", fd[0]);
                } else {
...
#endif
#if __linux__
        else if(strcmp(info.protocol, "tproxy") == 0) {
            has_tproxy = true;
            int fd[4] = {-1, -1, -1, -1};
            listenOption ops = {
            ...
            if(strcmp(info.hostname, "localhost") == 0) {
                if(ListenLocalhostTcp(info.port, fd, &ops) < 0) {
                    return -1;
                }
                if(ListenLocalhostUdp(info.port, fd+2, &ops) < 0) {
                    return -1;
                }
#ifdef HAVE_BPF
                ...
                fd[0] = ListenTcpD(&info, &ops);
                if (fd[0] < 0) {
                    return -1;
                }
                fd[2] = ListenUdpD(&info, &ops);
                if (fd[2] < 0) {
                    return -1;
                }
#ifdef HAVE_BPF
                if(opt.bpf_cgroup) {
                    if(strcmp(info.hostname, "[::]") == 0){
                        localhost4.sin_port = htons(info.port);
                        localhost6.sin6_port = htons(info.port);
                        if(load_bpf(opt.bpf_cgroup, &localhost4, &localhost6, (uint32_t)opt.bpf_fwmark)) {
                            return -1;
                        }
                    }else {
                        sockaddr_storage addr{};
                        storage_aton(info.hostname, info.port, &addr);
...
#if __linux__
#ifdef HAVE_BPF
    if(!has_tproxy && opt.bpf_cgroup && load_bpf(opt.bpf_cgroup, nullptr, nullptr, (uint32_t)opt.bpf_fwmark)) {
        return -1;
    }
#endif
    if(opt.tun_mode) {
