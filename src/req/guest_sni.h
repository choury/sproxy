...
class Guest_sni: public Guest{
    //外层预建的CONNECT请求，listen路径为空，嗅探完成后由forward合成
    std::shared_ptr<HttpReqHeader> req;
    std::list<Buffer> quic_init_packets;
    SSL_CTX* ctx = nullptr;
    size_t sniffer(Buffer&& bb);
    ReqStatus* forward(const char* sni, bool ech, Protocol prot, uint64_t id);
public:
    ...
    explicit Guest_sni(std::shared_ptr<RWer> rwer, std::shared_ptr<HttpReqHeader> req);
